        ...
        child: Column(
          children: [
            ...
            DropdownTextField<DropDownModel>(
              ...
              validate: (DropDownModel value) => value.validateDropDown(context),
              onChange: (DropDownModel model)=> filterData.onGovernChange(model),
              finData: (filter) async => await CustomerRepository(context).getGovernments(filterData.countryModel?.id,false),
            ),
            DropdownTextField<DropDownModel>(
              ...
              finData: (filter) async => await CustomerRepository(context).getCities(filterData.governModel?.id,false),
            ),
            DropdownTextField<DropDownModel>(
              ...
              hintColor: MyColors.blackOpacity,
              validate: (DropDownModel value) => value.validateDropDown(context),
              onChange: (DropDownModel model)=> filterData.onRegionChange(model),
              finData: (filter) async => await CustomerRepository(context).getRegions(filterData.cityModel?.id,false),
            ),
            DropdownTextField<CategoryModel>(
              ...
              textColor: MyColors.black,
              hintColor: MyColors.blackOpacity,
              validate: (CategoryModel value) => value.validateDropDown(context),
              onChange: (CategoryModel model)=> filterData.onCategoryChange(model),
              finData: (filter) async => await CustomerRepository(context).getCategories(filterData.regionModel?.id??0,false),
            ),
            DropdownTextField<SubCategoryModel>(
              ...
              onChange: (SubCategoryModel model)=> filterData.onSubCategoryChange(model),
              finData: (filter) async => filterData.catModel?.subCategory??<SubCategoryModel>[],
            ),
          ],
        ),
      ),
